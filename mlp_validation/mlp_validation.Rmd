---
title: "Predicting Microbial Load using 16S data"
...
```{r, training_data_comments_1, echo=F, results=F, plot=T, message=F, warning=F}

# check normality
data.frame(faecali = nishijima.training[,1]) %>%
  ggplot()+
  ggridges::geom_density_ridges(aes(x=faecali, y=1), fill="white", alpha=0.2)+
  theme_classic()+
  labs(title = colnames(nishijima.training)[1],
       x = paste(colnames(nishijima.training)[1], "Log Abundance", sep=" "),
       y="Frequency")
...
```{r, training_data_comments_2, echo=F, results=F, plot=T, message=F, warning=F}

# was the same value imputed for 0?
data.frame(pseudo = apply(nishijima.training, 1, min) ) %>%
  ggplot()+
  ggridges::geom_density_ridges(aes(x=pseudo, y=1), fill="white", alpha=0.2)+
  theme_classic()+
  labs(title = "Pseudocount distribution",
       x = "Minimum value",
       y="Frequency")
...
```{r, internal_validation_1, echo=F, results=F, plot=T, message=F, warning=F}
...
```{r, internal_validation_2, echo=F, results=F, plot=T, message=F, warning=F}

data.frame(obs = nishijima.model.predictions$obs) %>%
  ggplot()+
  ggridges::geom_density_ridges(aes(x=obs, y=1), fill="white", alpha=0.2)+
  theme_classic()+
  labs(title = "Microbial Load Distrubtion",
       x = "Measured Microbial Load",
       y="Frequency")
...
```{r, internal_validation_3, echo=F, results=F, plot=T, message=F, warning=F}

# Here's a scatter plot with embedded correlation

ggplot(nishijima.model.predictions, 
          aes(x = obs, y = ave.pred))+
  geom_point(shape=21, fill="white", size=2.5)+
  geom_smooth(method="lm", color="black")+
  ggpubr::stat_cor(method = "pearson")+
  theme_classic()+
  labs(x="Measured Microbial Load (log10)",
...
```{r, retrain_model_1, echo=FALSE, cache=F, results=FALSE, warning=FALSE, message=FALSE}
...
saveRDS(nishijima.model.rebuilt, "../saved/nishijima.model.rebuilt.Rds")
...
nishijima.model.rebuilt = readRDS("../saved/nishijima.model.rebuilt.Rds")
...
# Here's a scatter plot with embedded correlation
ggplot(nishijima.model.rebuilt.predictions, 
          aes(x = obs, y = ave.pred))+
  geom_point(shape=21, fill="white", size=2.5)+
  geom_smooth(method="lm", color="black")+
  ggpubr::stat_cor(method = "pearson")+
...
```{r, retrain_model_4}
# Lastly, check Pearson correlation
ggplot(testing.predictions.rebuilt, 
          aes(x = obs, y = pred))+
  geom_point(shape=21, fill="white", size=2.5)+
  geom_smooth(method="lm", color="black")+
  ggpubr::stat_cor(method = "pearson")+
...
```{r, loop_models, echo=F, results=FALSE, warning=FALSE, message=FALSE, cache=F}
...
saveRDS(model.loop, "../saved/mlp_model.loop.Rds")
t3 <- Sys.time()
t3 - t0

}

model.loop = readRDS("../saved/mlp_model.loop.Rds")
...
```{r, plot_loop_models_time, echo=FALSE, results=FALSE, plot=T, warning=FALSE, message=FALSE}
# check how long each model takes to build
ggplot(model.loop[,c("iter", "model", "time")] %>% distinct())+
  geom_boxplot(aes(x = model, y=time))+
  geom_point(aes(x =  model, y=time, fill= model), shape=21, size=3, alpha=1)+
...
```{r, plot_loop_models_rmse , echo=FALSE, results=FALSE, plot=T, warning=FALSE, message=FALSE}
ggplot(model.loop[,c("iter", "model", "internal.rmse")] %>% distinct())+
  geom_boxplot(aes(x = model, y=internal.rmse))+
  geom_point(aes(x =  model, y=internal.rmse, fill= model), shape=21, size=3, alpha=1)+
    scale_fill_manual(values=RColorBrewer::brewer.pal(name="Set3", n = length(ml.models)))+
  theme_classic()+theme(legend.position="none")+
...
```{r, plot_loop_models_pearson, echo=FALSE, results=FALSE, plot=T, warning=FALSE, message=FALSE}
# plot pearson values
model.loop.pearson.plot = ggplot(model.loop.pearson)+
  geom_boxplot(aes(x = model, y=pearson))+
  geom_point(aes(x = model, y=pearson, fill= model), shape=21, size=3, alpha=1)+
    scale_fill_manual(values=RColorBrewer::brewer.pal(name="Set3", n = length(ml.models)))+
  theme_classic()+theme(legend.position="none")+
  geom_hline(yintercept = 0, linetype=2, linewidth = 0.2, alpha=0.5)+
