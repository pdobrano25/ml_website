---
title: "Predicting Crohn's Disease using 16S Data from Biopsies"
---
...
Gevers et al's 2014 Cell Host and Microbe [study](https://pubmed.ncbi.nlm.nih.gov/24629344/) is one of the first and most highly cited studies on the microbiome in IBD.
...
# save a copy as shiny_meta, just once
# saveRDS(gevers_data, "machOmics/shiny_meta.Rds")
...
data.frame(depth = rowSums(gevers_16S)) %>%
  ggplot()+
  ggridges::geom_density_ridges(aes(x=depth, y=1), fill="white", alpha=0.2)+
  theme_minimal()+
...
# save this as shiny app data, just once
# saveRDS(gevers_16S_gen, "machOmics/shiny_data.Rds")
...
```{r, no_log, echo=F, results = F,plot=T, warning=FALSE, message=FALSE}
...
```{r, loop_models, echo = F, results=FALSE, warning=FALSE, message=FALSE}
...
saveRDS(gevers_model_loop, "../saved/gevers_model_loop.Rds")

}

gevers_model_loop = readRDS("../saved/gevers_model_loop.Rds")
...
ggplot(gevers_model_loop)+
  ...
  geom_point(aes(x =  model, y=AUC, fill= model), shape=21, size=3, alpha=1)+
