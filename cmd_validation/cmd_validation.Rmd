---
title: "Predicting IBD using Metagenomic Data from Several Cohorts"
---
...
Ning et al. (2023) perform a [meta-analysis](https://www.nature.com/articles/s41467-023-42788-0) of microbiome studies using the CMD to predict IBD from non-IBD controls.
...
```{r, helper_function, echo=F, warning=FALSE, message=FALSE}
...
auc_test <- pROC::roc(cmd_data[grepl(heldout, rownames(cmd_data)),]$study_condition,
                     predictions$IBD, levels=c("control", "IBD"))
if(plot == FALSE){
...
output = data.frame(
           auc = auc_test$auc,
           sens = caret::sensitivity(binary_preds$pred, binary_preds$obs, levels=c("control", "IBD")),
           spec = caret::specificity(binary_preds$pred, binary_preds$obs, levels=c("control", "IBD")),
           dataset = heldout,
           algorithm = model)
return(output)
}
...
auc_ci_test <- pROC::ci.se(auc_test, specificities = seq(0, 1, l = 100), boot.n = 2000, levels=c("control", "IBD"))
...
```{r, lld_hmp_unbalanced, results=F, plot=T, echo=FALSE, warning=FALSE, message=FALSE}

cached = T

if(cached == F){

lld_hmp_unbalanced = cmd_ml_pipeline(cmd_data_merge, 
                heldout = "NIE",
                balancing = FALSE,
                plot = TRUE)

saveRDS(lld_hmp_unbalanced, "../saved/lld_hmp_unbalanced.Rds")

}

lld_hmp_unbalanced = readRDS("../saved/lld_hmp_unbalanced.Rds")
...
```{r, lld_hmp_balanced, results=F, plot=T, echo=FALSE, warning=FALSE, message=FALSE}

cached = T

if(cached == F){

lld_hmp_balanced = cmd_ml_pipeline(cmd_data_merge, 
                heldout = "NIE",
                balancing = TRUE,
                plot = TRUE)

saveRDS(lld_hmp_balanced, "../saved/lld_hmp_balanced.Rds")

}

lld_hmp_balanced = readRDS("../saved/lld_hmp_balanced.Rds")
...
```{r, nie_lld, results=FALSE, echo=FALSE, warning=FALSE, message=FALSE}

cached = T

if(cached == F){

nie_lld = cmd_ml_pipeline(cmd_data_merge, 
                heldout = "HMP",
                balancing = TRUE,
                plot = TRUE)
  
saveRDS(nie_lld, "../saved/nie_lld.Rds")

}

nie_lld = readRDS("../saved/nie_lld.Rds")
...
```{r, nie_hmp, results=FALSE, echo=FALSE, warning=FALSE, message=FALSE}

cached = T

if(cached == F){
  
nie_hmp = cmd_ml_pipeline(cmd_data_merge, 
                heldout = "LLD",
                balancing = TRUE,
                plot = TRUE)

saveRDS(nie_hmp, "../saved/nie_hmp.Rds")

}

nie_hmp = readRDS("../saved/nie_hmp.Rds")

nie_hmp
  
```

These AUCs are less impressive than what were originally published [See Figure 2G](https://www.nature.com/articles/s41467-023-42788-0/figures/2).
...
```{r, loop_models, echo=F, results = F, warning=FALSE, message=FALSE}
...
saveRDS(cmd_model_loop, "../saved/cmd_model_loop.Rds")

}

cmd_model_loop = readRDS("../saved/cmd_model_loop.Rds")
...
ggplot(cmd_model_loop)+
  ...
  geom_point(aes(x =  algorithm, y=auc, fill= algorithm), color="black", shape=21,size=2, alpha=1)+
  theme_classic()+
...
ggplot(cmd_model_loop)+
  ...
  geom_point(aes(x =  algorithm, y=sens, fill= algorithm), shape=21, size=2, alpha=1)+
  scale_fill_manual(values=RColorBrewer::brewer.pal(name="Set3", n = length(ml.models)))+
...
ggplot(cmd_model_loop)+
  ...
  geom_point(aes(x =  algorithm, y=spec, fill= algorithm), shape=21, size=2, alpha=1)+
  scale_fill_manual(values=RColorBrewer::brewer.pal(name="Set3", n = length(ml.models)))+
  theme_classic()+
theme(legend.position="none",
                           axis.text.x=element_text(angle=45,vjust=1, hjust=1))+
